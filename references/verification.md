...
| 视频媒体 | MP4 和封面使用预期来源；文件未超过平台限制；完整解码通过；`video/mp4`、文件大小、Range 206、`Content-Range`、CSP、浏览器元数据和播放控件均已验证 |
...
| 部署版本 | 生产环境版本号和提交 ID 与已验证提交一致；不能用测试环境 100% 代替生产发布证据 |
...
验证视频时，不要下载整个大文件作为唯一检查。先读取 `HEAD`，再发送 `Range: bytes=0-1023`，最后在生产浏览器中读取时长、自然分辨率、`readyState` 和 `error`。这些证据共同覆盖静态文件、CDN 分段响应、CSP 与浏览器解码。
